...
import { useAllUsers, useAssignCompanyAdmin } from "@/hooks/super-admin/useSuperAdmin";
import { Users, Shield, Building2, Mail, Calendar, Search, Filter } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

export default function UsersManagement() {
  const { data: users, isLoading } = useAllUsers();
  const assignAdminMutation = useAssignCompanyAdmin();
  ...
  const handleAssignAdmin = async (userId, companyId) => {
    try {
      await assignAdminMutation.mutateAsync({ userId, companyId });
      toast({
        title: "Success",
  ...
  const filteredUsers = users?.filter(user => {
    ...
    const matchesCompany = companyFilter === "all" || 
                          (user.organizationId?._id === companyFilter);
    
    return matchesSearch && matchesRole && matchesCompany;
  }) || [];

  const companies = [...new Set(users?.map(u => u.organizationId).filter(Boolean))] || [];
