...
export default function CompaniesManagement() {
  ...
  const handleStatusChange = async (companyId, newStatus) => {
    try {
      await updateStatusMutation.mutateAsync({ companyId, isActive: newStatus });
      toast({
        title: "Success",
        description: `Company ${newStatus ? 'activated' : 'deactivated'} successfully`,
      });
    } catch (error) {
      toast({
        title: "Error",
            ...
            <div className="space-y-3 mb-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-600">{company.stats?.users || 0} users</span>
                </div>
                <div className="flex items-center gap-2">
                  <FolderOpen className="h-4 w-4 text-gray-400" />
                  <span className="text-gray-600">{company.stats?.projects || 0} projects</span>
                </div>
                <div className="flex items-center gap-2">
                  <FileText className="h-4 w-4 text-gray-400" />
                  <span className="text-gray-600">{company.stats?.tasks || 0} tasks</span>
                </div>
                <div className="flex items-center gap-2">
                  <Building2 className="h-4 w-4 text-gray-400" />
                  <span className="text-gray-600">{company.stats?.forms || 0} forms</span>
                </div>
              </div>
